...
| M1 `lh-core` | **done** — every §8 item is in place, and it has grown past them: probe, checksums, SBE, verify, scan, tool registry, conversion, torrent read/write/tracker list, job queue (J1, J2) done; SBE repair ([docs/sbe-repair.md](docs/sbe-repair.md), R1–R5) done for FLAC and WAV, including the TUI screen and in-place fixing; etree naming, tagging and rename ([docs/tagging.md](docs/tagging.md), N1–N3) done; the info `.txt` setlist ([docs/info-file.md](docs/info-file.md), I1) done; MP3 sample clips under a size limit (`lh sample`, [docs/sample.md](docs/sample.md)) done, through `lame` or `ffmpeg` |
...
GUI work in §4. The info `.txt` — setlist and track times from those tags — followed as
`lh setlist` ([docs/info-file.md](docs/info-file.md)).
...
- GitHub Actions release matrix for the three platforms (`.github/workflows/release.yml`).
  Releases ship `lh` and `lh-tui` only; the GUI joins once it is ready to ship.
- **No bundled tools, on any platform** (decided 2026-09-26). Users install `flac` (later
  `metaflac`, `shntool`) themselves: `apt`/`brew` on Linux and macOS, Xiph's build on
  Windows. Bundling would make us re-sign a third-party binary for notarization, own its
  security updates and mirror its GPL source in every release. Revisit for Windows only if
  users struggle; the registry's `tools/` sidecar lookup already supports it.
...
- **Licensing:** our code is MIT OR Apache-2.0. Nothing GPL ships with it, since the
  `flac`/`metaflac` tools are not bundled.
...
| **M4** | Packaging | CI matrix, installers, notarization, README and docs. | Medium; front-load the CI |
| — | **v0.1** | | |
...
3. ~~Bundle sidecars in the installer, or download on first run?~~ **Neither.** Users
   install the tools themselves (§7).
